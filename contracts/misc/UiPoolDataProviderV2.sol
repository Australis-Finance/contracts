...
import {IAustralisOracle} from './interfaces/IAustralisOracle.sol';
...
contract UiPoolDataProviderV2 is IUiPoolDataProviderV2 {
  ...
  {
    IAustralisOracle oracle = IAustralisOracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      reserveData.astTokenAddress = baseData.astTokenAddress;
    ...
    returns (UserReserveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].scaledAstTokenBalance = IAstToken(baseData.astTokenAddress)
